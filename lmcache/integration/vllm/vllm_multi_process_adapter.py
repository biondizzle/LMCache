...
class LMCacheMPWorkerAdapter:
    def __init__(
        ...
        *,
        ...
        # Retain the vLLM block size so we can ship it to the LMCache
        # server in ``register_kv_caches`` — the server uses it to
        # derive per-group compression ratios when some KV layer
        # groups compress multiple logical tokens into a single
        # physical slot (``shape_desc.bs < vllm_block_size``).
